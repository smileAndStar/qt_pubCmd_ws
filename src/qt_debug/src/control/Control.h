 ...
 * 
 * 这个类实现了 =按键控制= 和 =摇杆控制= 两种模式。
 * 这个类负责维护机器人的运动状态（如前进、后退）和速度值。
 * 当用户改变动作或速度时，它会计算出正确的速度指令并调用baseCmdPub的API发布出去。
 */
class Controller
{
public:
    /**
     * @brief Controller 的构造函数。
...
private:
     ...
     * 这是一个核心的私有函数，所有公共接口最终都会调用进行消息的更新发布。
     */
    void updateMovement();
